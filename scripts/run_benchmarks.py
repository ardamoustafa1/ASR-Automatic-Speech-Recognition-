...
def run_benchmarks():
    print("Running ASR-Pro NLP pipeline latency & throughput benchmarks...")
    ...
    try:
        ...
        print(f"Full NLP Pipeline execution time: {nlp_time:.4f}s")

        # Total execution
        total_time = nlp_time
        results["metrics"]["total_benchmark_time_sec"] = round(total_time, 4)
        print(f"Total benchmark execution time: {total_time:.4f}s")
    finally:
        db.close()
    ...
    artifact_path = os.path.join(output_dir, "benchmark_results.json")
    with open(artifact_path, "w") as f:
        json.dump(results, f, indent=2)

    print(f"Benchmark results saved to {artifact_path}")
