...
def main():
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        default="benchmarks/eval_dataset.json",
    ...
    parser.add_argument(
        ...
        help="Path to directory containing live WAV files (required for benchmarking).",
    )
    parser.add_argument(
        "--output-md", type=str, default="data/wer_report.md", help="Path to save markdown report."
    )
    parser.add_argument(
        "--output-json", type=str, default="data/wer_report.json", help="Path to save JSON report."
    )
    parser.add_argument(
        "--max-wer",
        type=float,
        default=None,
        help="Maximum acceptable Word Error Rate percentage. Fails with exit code 1 if exceeded.",
    )
    ...
    if args.max_wer is not None:
        if summary["overall_wer_percent"] > args.max_wer:
            print(
                f"❌ REGRESSION FAILED: Overall WER ({summary['overall_wer_percent']}%) exceeds maximum allowed threshold ({args.max_wer}%)."
            )
            sys.exit(1)
        else:
            print(
                f"✅ REGRESSION PASSED: Overall WER ({summary['overall_wer_percent']}%) is within threshold ({args.max_wer}%)."
            )
