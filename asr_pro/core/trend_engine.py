...
from asr_pro.db.session import SessionLocal, init_db as _core_init_db, session_scope
...
def dashboard_summary(db, window="7d"):
    ...
    """
    from asr_pro.db.models import Conversation, AlertEvent
    from datetime import datetime, timedelta, timezone
    ...
    active_alerts = db.query(AlertEvent).filter(AlertEvent.acknowledged == False, AlertEvent.created_at >= start_date).count()
    ...
    from asr_pro.core.trend_engine import get_trend_data, detect_anomalies
