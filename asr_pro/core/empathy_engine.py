...
EMPATHY_DICTIONARY = {
    "active_listening": [
        "sizi anlıyorum", "çok haklısınız", "dinliyorum", "teşekkür ederim", 
        "kesinlikle", "tabii ki", "evet anladım"
    ],
    "compassion": [
        "çok üzgünüm", "kusura bakmayın", "özür dileriz", "yaşadığınız durum", 
        "rahatsızlık için", "mağduriyet", "beklettiğim için"
    ],
    "solution": [
        "yardımcı olacağım", "hemen kontrol ediyorum", "birlikte çözeceğiz", 
        "halledeceğim", "işleminizi başlatıyorum", "hızla çözüyoruz", "çözüm sunacağım"
    ],
    "defensive": [
        "sistem böyle", "yapacak bir şey yok", "daha önce de söyledim", 
        "anlattığım gibi", "bizim hatamız değil", "prosedür gereği", "elimizden gelen bu"
    ]
}
...
def analyze_soft_skills(segments: Sequence[SegmentInput], agent_speaker_id: Optional[str] = None, use_ai: bool = True) -> EmpathyResult:
            ...
            for phrase in EMPATHY_DICTIONARY["defensive"]:
                if _fuzzy_match(phrase, text_lower):
                    if classifier and len(text.split()) > 3:
                        ...
                        res = classifier.predict(text, labels=["passive aggressive attitude", "technical explanation"], hypothesis=hyp)
                        scores = dict(zip(res["labels"], res["scores"]))
    ...
    final_score = base_score + score_active + score_compassion + score_solution + bonus_crisis - penalty_defensive - penalty_wpm - penalty_interruption
    ...
    summary_parts = []
    if final_score >= 85:
        summary_parts.append("🏆 Kusursuz İletişim (Top-Tier): Temsilci mükemmel bir empati sergiledi.")
    elif final_score >= 65:
        summary_parts.append("Temsilcinin iletişimi standart seviyede, iyileştirmeye açık alanlar var.")
    else:
        summary_parts.append("⚠️ Zayıf İletişim: Müşteriye karşı agresif, umursamaz veya savunmacı bir tutum sergilendi.")
        
    if bonus_crisis > 0:
        summary_parts.append("🚨 Kriz Yönetimi Başarılı: Müşteri öfkeliyken profesyonelce şefkat gösterildi (+Bonus).")
    if penalty_interruption > 0:
        summary_parts.append(f"🛑 Agresif Söz Kesme: Temsilci {interruption_count} kez müşterinin lafını bitirmesine izin vermedi (Büyük Ceza).")
    if penalty_wpm > 0:
        summary_parts.append(f"⏱️ Baştan Savma: Temsilci {high_wpm_segments} kez çok hızlı konuştu (Rushing).")
