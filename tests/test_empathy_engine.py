...
def test_empathy_high_score():
    segments = [
        SegmentInput(start=0.0, end=2.0, text="Merhaba, sizi anlıyorum.", segment_index=0, speaker="SPEAKER_01"),
        SegmentInput(start=2.5, end=5.0, text="Yaşadığınız durum için çok üzgünüm.", segment_index=1, speaker="SPEAKER_01"),
        SegmentInput(start=5.5, end=8.0, text="Hemen kontrol ediyorum ve yardımcı olacağım.", segment_index=2, speaker="SPEAKER_01")
    ]
...
def test_empathy_low_score_defensive():
    segments = [
        SegmentInput(start=0.0, end=2.0, text="Yapacak bir şey yok hanımefendi.", segment_index=0, speaker="SPEAKER_01"),
        SegmentInput(start=2.5, end=5.0, text="Size daha önce de söyledim, sistem böyle çalışıyor.", segment_index=1, speaker="SPEAKER_01")
    ]
...
def test_interruption_penalty():
    segments = [
        ...
        SegmentInput(start=0.0, end=3.0, text="Ben bunu kabul etmiyorum iptal edeceğim", segment_index=0, speaker="SPEAKER_00"),
        # Temsilci müşteri lafını bitirmeden 2.0. saniyede araya giriyor! (Overlap 1.0 saniye, > 0.2)
        SegmentInput(start=2.0, end=5.0, text="Sizi anlıyorum efendim ama dinleyin", segment_index=1, speaker="SPEAKER_01")
    ]
