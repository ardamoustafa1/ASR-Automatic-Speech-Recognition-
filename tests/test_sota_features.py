...
def test_ivr_segment_detection():
    ...
    assert (
        is_ivr_segment("Lütfen beklemeye devam ediniz, müşteri temsilcisine aktarıyorum.", 12.0)
        is True
    )
...
def test_semantic_role_guard_and_interruption():
    ...
    segments = [
        SegmentInput(
            ...
            text="Merhaba, Vodafone müşteri hizmetleri ben Arda, nasıl yardımcı olabilirim?",
            speaker="SPEAKER_01",
        ),  # Wrong assigned to customer
        SegmentInput(
            start=3.5,
            end=7.0,
            text="Faturam çok yüksek geldi, iptal ettirmek istiyorum şikayetçiyim!",
            speaker="SPEAKER_00",
        ),  # Overlap interruption + wrong assigned to agent
        SegmentInput(
            start=8.0,
            end=10.0,
            text="Sistemimize bakıyorum hemen kontrol ediyorum efendim.",
            speaker="SPEAKER_00",
        ),  # Correct agent
    ]
