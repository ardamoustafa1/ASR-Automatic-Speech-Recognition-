...
def test_get_conversations_pagination(client: TestClient):
    token = get_admin_token(client)
    response = client.get(
        "/api/v1/conversations?skip=0&limit=5", headers={"Authorization": f"Bearer {token}"}
    )
    assert response.status_code == 200
...
def test_404_not_found(client: TestClient):
    token = get_admin_token(client)
    response = client.get(
        "/api/v1/conversations/999999", headers={"Authorization": f"Bearer {token}"}
    )
...
def test_get_alerts_with_query(client: TestClient):
    token = get_admin_token(client)
    response = client.get(
        "/api/v1/alerts?acknowledged=false", headers={"Authorization": f"Bearer {token}"}
    )
    assert response.status_code == 200
    assert isinstance(response.json(), list)
