...
from asr_pro.core.compliance_engine import analyze_compliance_risk, ComplianceViolation

def test_finance_red_flag_fuzzy():
    segments = [
        SegmentInput(start=0.0, end=2.0, text="Merhaba hoş geldiniz.", segment_index=0),
        SegmentInput(start=2.0, end=5.0, text="Bu yatırım size kesin kazandırır merak etmeyin.", segment_index=1),
        SegmentInput(start=5.0, end=8.0, text="Kredi kart şifrenizi bana okur musunuz?", segment_index=2)
    ]
...
def test_negation_false_positive_shield():
    segments = [
        SegmentInput(start=0.0, end=3.0, text="Bizim fonlarımızda sıfır risk diye bir şey yoktur efendim.", segment_index=0),
        SegmentInput(start=3.0, end=6.0, text="Yatırımda kesin kazandırır demek yasaktır.", segment_index=1)
    ]
...
def test_negation_filter_complex():
    segments = [
        SegmentInput(start=0.0, end=3.0, text="Size bu konuda maalesef garanti vermiyoruz.", segment_index=0),
    ]
    violations = analyze_compliance_risk(segments, domain_key="finance", use_ai=False)
    assert len(violations) == 0
