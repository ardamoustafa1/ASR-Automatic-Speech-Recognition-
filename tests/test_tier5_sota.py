...
def test_voiceprint_biometrics():
    ...
    vec_a = BiometricService.extract_voiceprint(audio_a)
    vec_a2 = BiometricService.extract_voiceprint(audio_a)
    vec_b = BiometricService.extract_voiceprint(audio_b)
...
def test_mos_estimator():
    ...
    clean_audio[:24000] = 0.5 * np.sin(2 * np.pi * 300 * t[:24000]) + np.random.normal(0, 0.001, 24000)
...
def test_tier6_sota_diarization_upgrades():
    ...
    segs = [
        ...
        SegmentInput(start=2.5, end=3.5, text="efendim", speaker="SPEAKER_01"),  # 1 word ambiguous glitch
        SegmentInput(start=4.0, end=6.0, text="Size nasıl yardımcı olabilirim?", speaker="SPEAKER_00"),
    ]
    ...
    crosstalk_segs = [
        SegmentInput(start=10.0, end=14.0, text="Faturam çok yüksek geldi şikayetçiyim", speaker="SPEAKER_01"),
        SegmentInput(start=12.5, end=15.0, text="Hemen kontrol ediyorum beyefendi", speaker="SPEAKER_00"),
    ]
...
def test_acoustic_pitch_profiles_and_multi_speaker():
    ...
    segs = [
        SegmentInput(start=0.0, end=2.0, text="Merhaba Vodafone", speaker="SPEAKER_00"),
        SegmentInput(start=2.5, end=5.0, text="Faturamla ilgili şikayetim var", speaker="SPEAKER_01"),
        SegmentInput(start=5.5, end=8.0, text="Görüşmeye takım lideri olarak dahil oluyorum", speaker="SPEAKER_02"),
    ]
    diarizer = DiarizationService()
